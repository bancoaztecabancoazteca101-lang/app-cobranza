...
// FLORES es el identificador de la cartera en la fotografía, NO un apellido del cliente.
private val REGEX_FLORES = Regex("""(?i)(?<![A-ZÁÉÍÓÚÜÑ])FLORES(?![A-ZÁÉÍÓÚÜÑ])""")
...
fun normalizarCuPase(valor: String?): String = valor.orEmpty()
    .trim()
    .replace(Regex("\\s+"), "-")
    .replace(Regex("[^0-9-]"), "")
    .split('-')
...
private fun extraerNombre(bloque: String, cu: String): String {
    ...
    return if (numero > 0) tokens.take(numero).joinToString(" ")
    else tokens.takeLast(minOf(8, tokens.size)).joinToString(" ")
}

fun parsearFilasPaseFoto(textoOcr: String): List<PaseFotoFila> {
    val textoLimpio = limpiarOcr(textoOcr)

    // Filtro 1: la foto debe pertenecer a la cartera Flores.
    if (!REGEX_FLORES.containsMatchIn(textoLimpio)) return emptyList()
    ...
    for ((posicion, cu) in posicionesCu) {
        ...
        // FLORES valida la cartera; nunca se usa como apellido del cliente.
        if (!REGEX_FLORES.containsMatchIn(bloque)) continue
        ...
        resultado += PaseFotoFila(
            ...
            nombre = nombre,
            gcr = "Flores",
            contiene = finales.getOrNull(0)?.takeIf { it.length <= 40 },
            capitales = finales.getOrNull(1)?.takeIf { it.length <= 40 }
        )
    }
