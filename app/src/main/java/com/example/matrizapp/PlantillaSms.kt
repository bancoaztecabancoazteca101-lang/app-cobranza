...
interface PlantillaSmsDao {
     ...
     * una variante en la app dejándola en blanco sin tener que borrar la fila. TRIM evita que un
     * espacio suelto (que no es '' pero tampoco dice nada) cuente como variante activa. */
    @Query("SELECT * FROM plantilla_sms_table WHERE tipo = :tipo AND semana = :semana AND TRIM(texto) != '' ORDER BY slot")
    ...
    /** Convierte cualquier plantilla que haya quedado con el placeholder viejo {nombre}/{monto}
     * (versión de la app instalada entre el 30/08 antes de este fix) al estándar %nombre%/%monto%
     * que usa el resto de la app. Consulta idempotente — no hace nada si ya está migrado. */
    @Query("UPDATE plantilla_sms_table SET texto = REPLACE(REPLACE(texto, '{nombre}', '%nombre%'), '{monto}', '%monto%') WHERE texto LIKE '%{nombre}%' OR texto LIKE '%{monto}%'")
    suspend fun migrarPlaceholdersLegado()

    @Query("SELECT id FROM plantilla_sms_table WHERE tipo = :tipo AND semana = :semana AND slot = :slot LIMIT 1")
    suspend fun idDe(tipo: String, semana: Int, slot: Int): Long?
...
object PlantillasSemillaSms {
    ...
    /** Texto original de fábrica de un slot puntual — usado por "Restaurar esta semana" en la
     * pantalla de edición, sin tener que rehacer todas las plantillas de la app. */
    fun textoDeFabrica(tipo: String, semana: Int, slot: Int): String? {
        val mapa = if (tipo == "TT") textosTT else textosRef
        return mapa[semana]?.getOrNull(slot - 1)
    }
